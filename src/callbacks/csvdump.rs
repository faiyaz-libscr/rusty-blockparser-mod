...
impl Callback for CsvDump {
    ...
    fn on_block(&mut self, block: &Block, block_height: u64) -> OpResult<()> {
        // serialize block
        //self.block_writer.write_all(block.as_csv(block_height).as_bytes()).unswrap();

        // serialize transaction
        let block_hash = utils::arr_to_hex_swapped(&block.header.hash);
        for tx in &block.txs {
            //self.tx_writer.write_all(tx.as_csv(&block_hash).as_bytes())unswrap();
            ...
            for (i, output) in tx.value.outputs.iter().enumerate() {
                self.txout_writer
                    .write_all(output.as_csv(&txid_str, i as usize).as_bytes())?;
            }
